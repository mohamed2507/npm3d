...
from lib.trainer_denseunet import ContrastiveLossTrainer, HardestContrastiveLossTrainer, \
...
def get_trainer_denseunet(trainer):
...
def main(config, resume=False):
  ...
  Trainer = get_trainer_denseunet(config.trainer)
  trainer = Trainer(
      config=config,
